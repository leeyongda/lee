...
def write_article(title, tag, content, puplishd, now,):
    db.Article.insert(dict(uuid=get_uuid(), title=title, tag=tag,
...
def check_user_pass(user, pwd):
    if db.user.find_one({"username": user, "password": pwd}):
        return True
    return False
